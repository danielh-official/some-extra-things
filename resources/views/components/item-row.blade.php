...
<div class="flex gap-2 {{ $isProject ? 'items-start' : 'items-center' }} min-w-0 {{ $isSomeday ? 'opacity-50' : '' }}">
    @if ($isProject && $item->status === 'Open')
        <svg xmlns="http://www.w3.org/2000/svg" class="size-5 shrink-0 mt-0.5 text-[#706f6c] dark:text-[#A1A09A]"
    ...
    @elseif ($item->type === 'To-Do' && $item->status === 'Open')
        <svg xmlns="http://www.w3.org/2000/svg" class="size-4 shrink-0 text-[#706f6c] dark:text-[#A1A09A]"
